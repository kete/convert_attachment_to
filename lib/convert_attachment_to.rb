...
module Katipo
  module Acts #:nodoc:
    module ConvertAttachmentTo #:nodoc:
      ...
      module ClassMethods
        def convert_attachment_to(output_type, target_attribute)
          class_eval do
            ...
            @@configuration = { :output_type = output_type, :target_attribute = target_attribute, :max_pdf_pages => 10 }
            @@configuration[:is_versioned] = included_modules.include? ActiveRecord::Acts::Versioned ? true : false

            after_save :do_conversion

            extend Katipo::Acts::ConvertAttachmentTo::SingletonMethods
          end
      ...
      module InstanceMethods
        ...
        def do_conversion
          ...
          if @@acceptable_content_types.include?(type_to_convert)
            ...
            # save, but don't create a new version
            # if this model uses acts_as_versioned
            if @@configuration[:is_versioned]
